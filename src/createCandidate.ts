...
// set constant for email url
const baseCandidateUrlSend = 'https://api.recruitee.com/c/30789/mailbox/send';
...
router.post('/:position', store.single('resume'), async (req: Request, res: Response) => {
  ...
  try {
    ...
    // second form data section
  
    // send this email to the new user
    req.body =  { 
      "body_html" : "Hi,<br><br>Your application for Cube Academy has been submitted for a review. If your skills and experience are suitable for the role we will be in touch shortly.<br><br>Good Luck,<br><br>Cube Academy.", 
      "subject" : "Cube Academy Application", 
      "to": 
          [ 
            { 
              "email" : `${email}`, 
        ...
        // post the candiadte url with the new email 
        await recruitee.post(baseCandidateUrlSend, req.body);
